...
df = df.dropna(subset=['SSTA_AMO', 'sunspots'])
...
# === Detrend & standardize ===
amo_dt = signal.detrend(amo)
sun_dt = signal.detrend(sun)
amo_z = (amo_dt - np.mean(amo_dt)) / np.std(amo_dt, ddof=1)
...
# === Detect AMO maxima + minima ===
min_distance = 24  # months
amo_max_peaks, _ = find_peaks(amo_smooth, distance=min_distance, height=0)
amo_min_peaks, _ = find_peaks(-amo_smooth, distance=min_distance)
amo_peaks_all = np.sort(np.concatenate([amo_max_peaks, amo_min_peaks]))
amo_peak_dates = df.index[amo_peaks_all]
print(f"AMO peaks (max+min): {len(amo_peaks_all)}")

# === Cross-correlation: Sunspots → AMO ===
max_lag = 120  # months (look back)
lags = np.arange(0, max_lag+1)  # only positive lags (Sunspots lead AMO)
corrs = []

for lag in lags:
    c = np.corrcoef(amo_smooth[lag:], sun_smooth[:-lag])[0,1] if lag > 0 else np.corrcoef(amo_smooth, sun_smooth)[0,1]
    corrs.append(c)
...
print(f"Best lag (Sunspots → AMO): {best_lag} months, correlation = {best_corr:.3f}")

# === AR(1) surrogate function for AMO ===
...
n_surr = 1000
amo_surr = ar1_surrogate(amo_smooth, n_surr=n_surr)
surr_max_corrs = []

for surr in amo_surr:
    surr_corrs = []
    for lag in lags:
...
plt.xlabel("Lag (months, Sunspots → AMO)")
plt.ylabel("Correlation")
plt.title("Cross-Correlation: Sunspots lead AMO")
plt.legend()
plt.tight_layout()
plt.show()
...
plt.xlabel("Maximum correlation (Sunspots → AMO)")
