  ...
  return (
    <div className="w-full h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex flex-col">
      ...
      <div className="flex-1 p-6 relative">
        ...
        {/* Pinned point info */}
        {pinnedPoint && (
          <div className="absolute top-8 left-8 bg-black/90 backdrop-blur-md p-4 rounded-lg border border-yellow-500/50 shadow-2xl">
            <div className="flex items-center gap-2 mb-2">
              <Crosshair className="w-4 h-4 text-yellow-400" />
              <h3 className="text-yellow-400 font-bold">Pinned Point</h3>
            </div>
            <div className="space-y-1 text-sm font-mono">
              <div className="text-cyan-300">n = {pinnedPoint.n}</div>
              <div className="text-cyan-300">Ω(n) = {omegaCache[pinnedPoint.n]}</div>
              <div className="text-gray-400 text-xs mt-2">
                {primeSet.has(parseInt(pinnedPoint.n)) ? '✓ Prime (irreducible)' : '✗ Composite'}
              </div>
              <div className="text-gray-400 text-xs">
                {MODULI.map(m => parseInt(pinnedPoint.n) % m === 0 ? `${m}` : null).filter(Boolean).join(', ') || 'No divisors in {2,3,5,7,11}'}
              </div>
            </div>
            <button
      ...
      <div className="p-6 bg-black/30 backdrop-blur-md border-t border-white/10">
        <div className="max-w-5xl mx-auto space-y-4">
          ...
          <div className="flex items-center justify-center gap-2 text-sm flex-wrap">
            <span className="text-gray-400">Interesting regions:</span>
            {[1, 1000, 3000, 5000, 8190, 9210, 9400].map(val => (
              <button
                key={val}
                onClick={() => setPosition(Math.max(1, Math.min(val, END - windowWidth)))}
                className="px-4 py-1 bg-white/5 hover:bg-white/10 border border-white/10 rounded text-gray-300 transition-all duration-200 font-mono text-xs"
              >
                {val}
              </button>
            ))}
